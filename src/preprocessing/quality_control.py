"""Module for quality control of medical images."""
import json
...
class QualityControl:
    """Class for performing quality control on medical images."""

    def __init__(self, config: dict):
        self.config = config
        self.qc_report = {}

    def run(self, image, img_path=None):
        ...
        try:
            ...
            # save the report
            qc_path = self.config.get("qc_report_path", "qc_report.json")
    ...
    def _check_resolution(self, img):
        ...
        self.qc_report["resolution"] = {
            "voxel_size": str(voxel_size),
            "aspect_ratios": str(aspect_ratios)
        }
    ...
    def _check_intensity_properties(self, data):
        ...
        max_outlier_pct = self.config.get("max_outlier_percentage", 0.01)
        if outlier_percentage > max_outlier_pct:
            raise ValueError(
                f"Image has high percentage of outliers: {outlier_percentage:.2%}"
            )

        self.qc_report["intensity"] = {
            **stats_dict,
            "outlier_percentage": str(outlier_percentage)
        }
    ...
    def _check_orientation(self, img):
        ...
        if orientation != expected_orientation:
            print(f"Warning: Image orientation {orientation} differs "
                  f"from expected {expected_orientation}")
        self.qc_report["orientation"] = {
            "current": str(orientation),
            "expected": str(expected_orientation)
        }
